...
module.exports = class Bme680 {
    ...
    async initialize() {
        ...
        if (this.chip_variant !== constants.VARIANT_LOW && this.chip_variant !== constants.VARIANT_HIGH) {
            throw new Error(`Invalid variant ${this.chip_variant}`)
        }

        bme680Data.chip_id = constants.CHIP_ID;
        bme680Data.chip_variant = this.chip_variant;
    ...
    async getSensorData() {
        ...
        const adc_gas_res_low = (regs[13] << 2) | (regs[14] >> 6);
        const adc_gas_res_high = (regs[15] << 2) | (regs[16] >> 6);

        const gas_range_l = regs[14] & constants.GAS_RANGE_MSK;
        const gas_range_h = regs[16] & constants.GAS_RANGE_MSK;

        if (this.chip_variant === constants.VARIANT_HIGH) {
            bme680Data.data.status |= regs[16] & constants.GASM_VALID_MSK;
            bme680Data.data.status |= regs[16] & constants.HEAT_STAB_MSK;
        } else {
        ...
        bme680Data.ambient_temperature = this.ambient_temperature;
        ...
        if (this.chip_variant === constants.VARIANT_HIGH) {
            bme680Data.data.gas_resistance = await this.calcGasResistanceHigh(adc_gas_res_high, gas_range_h);
        } else {
            bme680Data.data.gas_resistance = await this.calcGasResistanceLow(adc_gas_res_low, gas_range_l);
        }

        bme680Data.calibration_data = this.calibrationData;
    ...
    async setPowerMode(value, blocking = false, maxPollTimeMs = null) {
        ...
        bme680Data.power_mode = this.power_mode;
    ...
    async  writeByte(cmd, byte) {
        return new Promise((resolve, reject) => {
            wire.writeByte(this.i2cAddress, cmd, byte, function (err, data) {
                if (err) {
                    reject(err);
                }
                resolve(data);
    ...
    async  readByte(cmd, length) {
        return new Promise((resolve, reject) => {
            if (!length || length === 1) {
                wire.readByte(this.i2cAddress, cmd, (err, config) => {
                    if (err) {
                        reject(err);
                    }
                    resolve(config);
                ...
                wire.readI2cBlock(this.i2cAddress, cmd, length, buffer, (err, config) => {
                    if (err) {
                        reject(err);
                    }
                    resolve(buffer);
